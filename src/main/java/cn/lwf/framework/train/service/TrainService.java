...
//    }
    ...
    public void syncTrainListByStation() {
        try{
            ...
            for (Train curTrain: trainList) {
                Train train = trainDao.findTrainByCode(curTrain.getTrainCode());
                if(train == null){
                    train = new Train();
                    train.setTrainCode(curTrain.getTrainCode()); //车次',
                    train.setStartStationName(curTrain.getStartStationName()); //始发站',
                    train.setEndStationName(curTrain.getEndStationName()); //终点站',
                    train.setTrainNo(curTrain.getTrainNo()); //车次编码',
                    train.setRunDate(curTrain.getRunDate()); //运行时间',
                    train.setTrainClassCode(curTrain.getTrainClassCode()); //车次类型号
                    train.setTrainClassName(curTrain.getTrainClassName()); //车次类型
